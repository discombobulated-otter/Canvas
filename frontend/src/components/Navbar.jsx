...
const NavBar = ({save,setSave}) => {
  ...
  const [showProjects, setShowProjects] = useState(false); // State for toggling the Projects dropdown
  ...
  useEffect(() => {
    const syncUser = async () => {
      ...
      try {
        const token = await session.getToken(); // get the JWT token
        ...
        {isOpen && (
          ...
          >
            <motion.div initial={{y:-100}} animate={{y:0}} transition={{
              ...
              }} className='hover:text-gray-200'>Home</NavLink>
              ...
              <NavLink to='/canvas' onClick={()=>{
                 setShowProjects(false)
              }}  className='hover:text-gray-200'>New Canvas</NavLink>
              <NavLink to='/joinCanvas' onClick={()=>{
                 setShowProjects(false)
              }}  className='hover:text-gray-200'>join canvas</NavLink>
            </motion.div>
